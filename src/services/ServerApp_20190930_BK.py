...
def get_customer_level_by_request_id(tasks, request_id):
    for task in tasks:
        if int(task['request_id']) == int(request_id):
            return task['manual_priority'], task['number_emp_needed'], task['sub_type_1'], task['reason_out_case_desc'], \
                   task['appointmentdate'], task['sub_type_2'], task['emp_speciallized'], task['contract'], task[
                       "date_confirmed"]
        ...
        for block in blocks:
            ...
            for file in files:
                ...
                for resource in resources:
                    ...
                    employee["emp_id"] = resource["emp_id"]
                    employee["emp_assigned"] = resource["emp_assigned"]
                    # employee["emp_assigned"] = count_assigned_task_emp(resource["emp_id"], parentFolder, block)
                    # print(employee["emp_assigned"])
                    employee["emp_type"] = resource["type"]
...
def count_assigned_task_emp(emp_id, parentFolder, block_id):
    command = 'grep -E "\\"assigned\\"\s*\:\s*\\"1\\"" -rl ./output/' + parentFolder + "/*/" + block_id + "/ > assigned_task_emp.txt"
    os.system(command)
    paths = []
